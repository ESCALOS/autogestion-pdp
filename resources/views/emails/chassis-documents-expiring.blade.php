...
<html lang="es">
    <head>
        ...
        <title>Documentos de Chassis Próximos a Vencer</title>
        <style>
            body {
    ...
    <body>
        <div class="header">
            <h1>🚚 Documentos de Chassis Próximos a Vencer</h1>
            <p>
        ...
        <div class="alert">
            ...
            Los siguientes chassis tienen documentos que vencen en {{ $daysToExpiration }} días. Es necesario renovar
            estos documentos para evitar interrupciones en las operaciones.
        </div>

        @foreach ($chassis as $chassisItem)
            <div class="chassis-item">
                <h3>🚚 Placa: {{ $chassisItem['license_plate'] }}</h3>
                <p>
                    <strong>Tipo de Vehículo:</strong>
                    {{ App\Enums\VehicleTypeEnum::tryFrom($chassisItem['vehicle_type'])->getLabel() }}
                </p>
                <p>
                    <strong>Cantidad de Ejes:</strong>
                    {{ $chassisItem['axle_count'] }}
                </p>
                <p>
                    <strong>Peso Seguro:</strong>
                    {{ $chassisItem['safe_weight'] }} kg
                </p>

                <div class="document-list">
                    <h4>Documentos que vencen:</h4>
                    @foreach ($chassisItem['documents'] as $document)
                ...
                @if (isset($chassisItem['appeal_token']))
                    <p style="margin-top: 15px">
                        <a
                            href="{{ route('chassis.appeal.show', $chassisItem['appeal_token']) }}"
        ...
        <div style="background-color: #e7f3ff; padding: 20px; border-radius: 8px; margin: 20px 0">
            ...
            <ul>
                ...
                <li>Hacer clic en los enlaces de actualización para cada chassis</li>
