...
class TruckAppealController extends Controller
{
    public function show(string $token)
    {
        $truck = Truck::where('appeal_token', $token)
            ->where('appeal_token_expires_at', '>', now())
            ->with(['documents', 'company'])
            ->firstOrFail();

        // Obtener documentos rechazados o vencidos
        $rejectedDocuments = $truck->documents()
        ...
        return view('truck-appeal', compact('truck', 'rejectedDocuments', 'token'));
    }
        ...
        try {
            ...
            foreach ($rejectedDocuments as $document) {
                ...
                if ($request->hasFile($fieldName)) {
                    ...
                    $filename = $truck->license_plate . '_' . $document->type->value . '_' . time() . '.' . $file->getClientOriginalExtension();
            ...
            // Actualizar estado del vehículo a revisión de documentos y limpiar token
            $truck->update([
            ...
            return redirect()->route('truck.appeal.success');
    ...
    {
        return view('truck-appeal-success');
    }
}
