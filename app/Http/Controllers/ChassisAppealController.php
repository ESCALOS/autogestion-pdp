...
class ChassisAppealController extends Controller
{
    public function show(string $token)
    {
        $chassis = Chassis::where('appeal_token', $token)
            ->where('appeal_token_expires_at', '>', now())
            ->with(['documents', 'company'])
            ->firstOrFail();

        // Obtener documentos rechazados o vencidos
        $rejectedDocuments = $chassis->documents()
        ...
        return view('chassis-appeal', compact('chassis', 'rejectedDocuments', 'token'));
    }
        ...
        try {
            ...
            foreach ($rejectedDocuments as $document) {
                ...
                if ($request->hasFile($fieldName)) {
                    ...
                    $filename = $chassis->license_plate . '_' . $document->type->value . '_' . time() . '.' . $file->getClientOriginalExtension();
            ...
            return redirect()->route('chassis.appeal.success');
    ...
    {
        return view('chassis-appeal-success');
    }
}
