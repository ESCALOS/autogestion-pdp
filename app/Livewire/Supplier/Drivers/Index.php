...
final class Index extends Component implements HasActions, HasSchemas, HasTable
{
    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ->query(SupplierDriver::query()->where('supplier_id', $supplier->id))
            ->columns([
                TextColumn::make('id')
                    ->label('ID')
                    ->sortable(),
            ...
            ->recordActions([
                ActionGroup::make([
                    Action::make('update_documents')
                        ...
                        ->color('info')
                        ->visible(fn (SupplierDriver $record): bool => $record->documents()->exists())
                        ->schema(function (SupplierDriver $record): array {
                            ...
                            foreach ($documents as $document) {
                                ...
                                $formSchema = [
                                    FileUpload::make("document_{$document->id}")
                                        ...
                                        ->maxSize(5120)
                                        ->directory(fn () => "PROVEEDORES/{$record->supplier->ruc}/DRIVERS/{$record->document_number}")
                        ...
                        ->modalWidth('2xl')
                        ->action(function (SupplierDriver $record, array $data): void {
                            try {
                    ...
                    Action::make('add_mercancias_course')
                        ...
                        ->color('success')
                        ->visible(fn (SupplierDriver $record): bool => in_array($record->status, [EntityStatusEnum::ACTIVE, EntityStatusEnum::PENDING_APPROVAL]) &&
                            ...
                            Grid::make(1)
                                ->schema([
                                    FileUpload::make('mercancias_document')
                                        ...
                                        ->maxSize(5120)
                                        ->required()
                                        ->directory(fn (SupplierDriver $record) => 'PROVEEDORES/'.$record->supplier->ruc."/DRIVERS/{$record->document_number}")
    ...
    {
        return view('livewire.supplier.drivers.index');
    }
}
