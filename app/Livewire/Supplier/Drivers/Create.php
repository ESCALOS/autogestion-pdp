...
namespace App\Livewire\Supplier\Drivers;
...
#[Title('Crear Conductor - Proveedor')]
                    ...
                    Step::make('driver_data')
                        ...
                        ->schema([
                            ...
                            TextInput::make('document_number')
                                ...
                                ->maxLength(20)
                                ->regex('/^\S+$/')
                                ->unique('supplier_drivers', 'document_number', modifyRuleUsing: function ($rule) use ($supplier) {
                                    return $rule->where('supplier_id', $supplier->id);
                                })
                                ->validationMessages([
                                    'unique' => 'Ya existe un conductor con este número de documento en tu proveedor.',
                            ...
                            TextInput::make('license_number')
                                ...
                                ->regex('/^\S+$/')
                                ->unique('supplier_drivers', 'license_number', modifyRuleUsing: function ($rule) use ($supplier) {
                                    return $rule->where('supplier_id', $supplier->id);
                                })
                                ->validationMessages([
                                    'unique' => 'Ya existe un conductor con este número de licencia en tu proveedor.',
                                    ...
                                    Grid::make(3)
                                        ->schema([
                                            FileUpload::make('documents.dni.file')
                                                ...
                                                ->disk('public')
                                                ->directory(fn () => 'PROVEEDORES/'.$supplier->ruc."/DRIVERS/{$this->data['document_number']}")
    ...
    public function create(): void
    {
        try {
            ...
            DB::transaction(function () use ($data, $supplier) {
                // Crear el conductor del proveedor
                $driver = SupplierDriver::create([
                ...
                foreach ($documentTypes as $key => $type) {
                    if (isset($data['documents'][$key]['file']) && $data['documents'][$key]['file']) {
                        $documentData = [
                            'documentable_type' => SupplierDriver::class,
            ...
            $this->reset();
            $this->form->fill();
            $this->redirect(route('supplier.drivers.index'), navigate: true);
            ...
            Notification::make()
                ...
                ->danger()
                ->send();
            Log::error('Error creating supplier driver: '.$e->getMessage());
        }
    }

    public function render(): View
    {
        return view('livewire.supplier.drivers.create');
    }
}
