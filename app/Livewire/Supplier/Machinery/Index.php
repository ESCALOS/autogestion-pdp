...
final class Index extends Component implements HasActions, HasSchemas, HasTable
{
    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ->query(SupplierMachinery::query()->where('supplier_id', $supplier->id))
            ->columns([
                TextColumn::make('id')
                    ->label('ID')
                    ->sortable(),
            ...
            ->filters([
                ...
                SelectFilter::make('nationality')
                    ->label('Nacionalidad')
                    ->options(function () use ($supplier) {
                        return SupplierMachinery::query()
            ...
            ->recordActions([
                ActionGroup::make([
                    Action::make('update_documents')
                        ...
                        ->color('info')
                        ->visible(fn (SupplierMachinery $record): bool => $record->documents()->exists())
                        ->schema(function (SupplierMachinery $record): array {
                            ...
                            foreach ($documents as $document) {
                                ...
                                $formSchema = [
                                    FileUpload::make("document_{$document->id}")
                                        ...
                                        ->maxSize(5120)
                                        ->directory(fn () => "PROVEEDORES/{$record->supplier->ruc}/MACHINERY/{$record->license_plate}")
                                ...
                                // Los vehículos solo usan expiration_date, no course_date
                        ...
                        ->modalHeading('Actualizar Documentos de la Maquinaria')
                        ->modalDescription('Actualiza los documentos de la maquinaria. Los marcados como obligatorios deben ser actualizados.')
                        ->modalSubmitActionLabel('Guardar Cambios')
                        ->modalWidth('2xl')
                        ->action(function (SupplierMachinery $record, array $data): void {
                            try {
                                ...
                                Notification::make()
                                    ...
                                    ->body('Los documentos han sido actualizados. La maquinaria está pendiente de aprobación.')
                                    ->success()
                                    ->send();
                    ...
                    Action::make('add_bonus')
                        ...
                        ->color('success')
                        ->visible(fn (SupplierMachinery $record): bool => in_array($record->status, [EntityStatusEnum::ACTIVE, EntityStatusEnum::PENDING_APPROVAL]) &&
                            ...
                            Grid::make(1)
                                ->schema([
                                    FileUpload::make('bonus_document')
                                        ...
                                        ->maxSize(5120)
                                        ->required()
                                        ->directory(fn (SupplierMachinery $record) => 'PROVEEDORES/'.$record->supplier->ruc."/MACHINERY/{$record->license_plate}")
                        ...
                        ->modalDescription('Completa los datos del documento de bonificación para esta maquinaria.')
                        ...
                        ->action(function (SupplierMachinery $record, array $data): void {
                            try {
                                ...
                                Notification::make()
                                    ...
                                    ->body('El documento de bonificación ha sido agregado y la maquinaria está pendiente de aprobación.')
                                    ->success()
                                    ->send();
    ...
    {
        return view('livewire.supplier.machinery.index');
    }
}
